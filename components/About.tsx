...
export default function About() {
  return (
    <section id="about" className="py-16 sm:py-24" style={{ backgroundColor: '#f4f7ff' }}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-start">
          ...
          <div className="flex flex-col gap-5">
            {instructors.map((instructor) => (
              <div
                key={instructor.name}
                className="light-card rounded-xl p-6 flex flex-col sm:flex-row gap-5"
              >
                ...
                <div className="flex flex-col gap-3">
                  <div>
                    <h3 className="font-sans font-semibold text-xl text-[#0a0f1e]">
                      {instructor.name}
                    </h3>
                    <p className="font-sans text-sm font-semibold text-[#d4a91a] mt-0.5">
                      {instructor.title}
                    </p>
                    <p className="font-mono text-xs text-[#7a8aaa] mt-1">{instructor.years}</p>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {instructor.specialties.map((spec) => (
                      <span
                        key={spec}
                        className="font-sans text-[#3a4a6a] text-xs px-2.5 py-1 rounded"
                        style={{
                          background: '#eef2ff',
