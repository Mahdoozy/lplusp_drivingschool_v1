...
export default function ThankYouPage() {
  return (
    <div className="min-h-screen py-16 px-4" style={{ backgroundColor: '#f4f7ff' }}>
      <div className="max-w-lg mx-auto flex flex-col gap-8">
        ...
        <div className="text-center flex flex-col items-center gap-4">
          ...
          <h1 className="font-syne font-bold text-3xl text-[#0a0f1e]">
            Thanks — we&apos;ll be in touch within the hour
          </h1>
          <p className="font-sans text-[#3a4a6a] text-base">
            Mick or Sidra will call or WhatsApp you to lock in your lesson time.
          </p>
        </div>
        ...
        >
          <h2 className="font-syne font-bold text-lg text-[#0a0f1e]">What happens next</h2>
          <ul className="flex flex-col gap-3">
            {[
              { num: '1', text: 'We confirm your slot — usually within the hour, Mon–Sat 6 am–6 pm' },
              { num: '2', text: 'You\'ll get a calendar invite and a WhatsApp reminder the day before' },
              { num: '3', text: 'Mick or Sidra picks you up from your door at lesson time — no need to travel' },
            ].map((item) => (
              <li key={item.num} className="flex items-start gap-3">
                <div
                  className="w-7 h-7 rounded-full flex-shrink-0 flex items-center justify-center font-syne font-bold text-sm text-[#0a0f1e]"
                  style={{ background: '#F5C842' }}
                >
                  {item.num}
                </div>
                <span className="font-sans text-[#3a4a6a] text-sm leading-relaxed pt-0.5">{item.text}</span>
              </li>
            ))}
          </ul>
        </div>
        ...
        >
          <div className="flex flex-col gap-1">
            <h2 className="font-syne font-bold text-xl text-[#0d1b3e]">
        ...
        <div className="text-center">
          <Link
            href="/"
            className="font-sans text-[#3a4a6a] text-sm hover:text-[#0a0f1e] transition-colors underline underline-offset-4"
          >
            ← Back to home
          </Link>
        </div>
